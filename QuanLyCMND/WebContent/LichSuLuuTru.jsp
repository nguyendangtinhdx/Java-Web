        ...
        <table class="table table-hover" style="width: 100%; margin: auto;">
            ...
            <tbody>
                    ...
                    if(request.getAttribute("ListLichSuLuuTru") != null){
                        ArrayList<LichSuLuuTruBEAN> ls = (ArrayList<LichSuLuuTruBEAN>)request.getAttribute("ListLichSuLuuTru");
                        for(LichSuLuuTruBEAN cd : ls){
                    %>
                    <tr>
                    <td><%=cd.getCmndID() %></td>
                    <td><%=cd.getHoKhauID() %></td>
                    <td><%=cd.getLoaiLuuTruID() %></td>
                    <td><%=cd.getThoiGianBatDauLT()%></td>
                    <td><%=cd.getThoiGianKetThucLT() %></td>
                    <td><%=cd.getQuanHeVoiChuHo() %></td>
                    <td><a href="LichSuLuuTruServlet?loai=xoa&tgbd=<%=cd.getThoiGianBatDauLT() %>"><img src="image/icon_delete.png"></a></td> --%>
                    </tr>
                    <%
                        }
                    }
